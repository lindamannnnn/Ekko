...
# ===================================================================
#  教师标签语义分析 —— 防止「自定义标签」（如「动手能力差」）被弱模型漏掉
#  思路：判断每个 tag 的语义倾向 + 自动给出"回扣落点"指引
#  两处 prompt 拼装都共用这些函数。
# ===================================================================
_NEG_KW = {"差", "弱", "不", "欠", "缺", "难", "散", "急", "乱", "慌", "粗",
           "不够", "不足", "缓慢", "欠缺", "问题", "困难", "差劲", "潦草", "马虎",
           "调皮", "内向", "慢", "生疏", "退步", "吃力", "混淆"}
# 2026-08-07 修复「注意力集中被判为中性、LLM 用"虽然...但..."转折稀释」：
# 把"集中/专心/全神贯注/聚精会神"等明确纳入正向词，且把"不错/到位/仔细"也补进来。
_POS_KW = {"好", "优", "棒", "积极", "认真", "专注", "突出", "强", "快", "精彩",
           "流畅", "扎实", "优秀", "优异", "进步", "提升", "完美", "稳定", "耐心",
           "自信", "大方", "细心",
           "集中", "专心", "全神贯注", "聚精会神", "投入", "沉浸", "沉浸式",
           "不错", "到位", "仔细", "沉着", "敏捷", "灵动", "沉稳"}
# 常见关键词 → 回扣到哪个"行为/维度"（避免 LLM 看到"动手能力差"不知道该写什么）
_FOCUS_MAP = {
    "动手": "动手操作能力（剪刀/胶棒/笔触控制/精细动作等实操环节）",
    "动手能力": "动手操作能力（剪刀/胶棒/笔触控制/精细动作等实操环节）",
    "注意力": "专注度（在需细致的环节是否走神/左顾右盼/被外界干扰）",
    "注意力集中": "专注度（全程注视老师示范/不与同学闲聊/不摆弄学具/起笔不犹豫/能持续 X 分钟不分神）",
    "注意力不集中": "专注度（容易走神的环节/对外界什么刺激敏感）",
    "走神": "专注度（容易走神的环节/对外界什么刺激敏感）",
    "分心": "专注度（分心诱因/具体场景）",
    "练习": "练习量（具体方向——哪个动作/题型/步骤要多练）",
    "加强练习": "练习量（具体方向——哪个动作/题型/步骤要多练）",
    "需加强练习": "练习量（具体方向——哪个动作/题型/步骤要多练）",
    "概念混淆": "概念清晰度（具体哪些概念易混/哪个定义/公式没掌握）",
    "概念": "概念清晰度",
    "混淆": "概念清晰度（具体哪些易混）",
    "需巩固": "巩固方向（哪些知识点/技能需回家回顾/再做一遍）",
    "巩固": "巩固方向",
    "需提升": "可提升点（具体哪个维度的提升空间）",
    "互动": "互动积极性（举手/回答/同伴协作的具体表现）",
    "积极互动": "互动积极性（举手/回答/同伴协作的具体表现）",
    "积极": "互动积极性",
    "表现优异": "表现优异点（哪个维度——完成度/创意/合作/毅力等）",
    "优异": "表现优异点",
    "优秀": "表现优异点",
    "表现": "整体表现",
    "逻辑清晰": "逻辑清晰度（步骤/因果/条理的具体表现）",
    "逻辑": "逻辑清晰度",
    "条理": "条理性",
    "乐于助人": "助人行为（具体帮了谁/做了什么）",
    "助人": "助人行为",
    "创意十足": "创意点（具体哪个作品元素/构思/配色有创意）",
    "创意": "创意点",
    "耐心": "耐心程度（哪些需要反复练的环节表现如何）",
    "构图": "构图能力",
    "色彩": "色彩运用",
    "线条": "线条运用",
    "造型": "造型能力",
...
# 正向标签的"必出现"具体行为关键词（按维度）——防止弱模型用"虽然...但..."转折稀释，
# 必须从下列清单里至少字面出现 1-2 个具体行为关键词，否则视为不合格。
# 关键词按"专注度/互动积极性/整体表现/学习态度"四个最常被点选的维度组织。
_POSITIVE_BEHAVIOR_HINT = {
    "专注度": (
        "至少字面出现以下 1-2 个正向行为关键词：\n"
        "    · 眼睛紧跟老师示范 / 临帖时眼睛不离开字帖 / 与老师同步看\n"
        "    · 起笔不犹豫 / 下笔连贯 / 收笔到位 / 笔不离纸\n"
        "    · 全程不左顾右盼 / 不与同学闲聊 / 不摆弄学具\n"
        "    · 能持续 X 分钟不分神 / 认真听讲不打断 / 跟着老师节奏走"
    ),
    "互动积极性": (
        "至少字面出现以下 1-2 个正向行为关键词：\n"
        "    · 主动举手回答 / 抢答 / 大声答到 / 主动提问\n"
        "    · 主动上台示范 / 主动给同学讲解 / 主动展示作品\n"
        "    · 主动与同伴协作 / 帮助同学 / 配合默契 / 邀请同伴一起"
    ),
    "整体表现": (
        "至少字面出现以下 1-2 个正向行为关键词：\n"
        "    · 坐姿端正 / 握笔正确 / 抬头挺胸 / 课堂常规稳定\n"
        "    · 跟随老师节奏 / 完成度高 / 主动加练 / 不需老师提醒"
    ),
    "学习态度": (
        "至少字面出现以下 1-2 个正向行为关键词：\n"
        "    · 认真听讲 / 仔细观察 / 主动思考 / 遇到困难不退缩\n"
        "    · 勇于尝试 / 敢于表达 / 虚心接受建议 / 勤于记录要点"
    ),
}
...
def _infer_focus(tag: str) -> str | None:
    """从标签里挑最长的已知关键词，返回对应的'落点'说明。"""
    if not tag:
        return None
    t = str(tag).strip()
    if t in _FOCUS_MAP:
        return _FOCUS_MAP[t]
    keys = sorted(_FOCUS_MAP.keys(), key=len, reverse=True)
    for k in keys:
        if k in t:
            return _FOCUS_MAP[k]
    return None
...
def _teacher_signal_lines(tags, note) -> str:
    """生成教师信号的逐项分析行（含语义倾向 ↘/↗/· 与回扣落点）。"""
    lines = []
    for raw in tags or []:
        tag = str(raw).strip()
        if not tag:
            continue
        sentiment = _classify_tag(tag)
        focus = _infer_focus(tag)
        if sentiment == "negative":
            ...
            arrow = "· 中性"
        focus_text = f" → 回扣落点：{focus}" if focus else "（无固定落点，按本节课实际表现自然回扣）"
        lines.append(f"  {arrow} 「{tag}」{focus_text}")
        # 2026-08-07：正向标签额外追加"必须字面出现"的具体行为关键词清单，
        # 防止弱模型用"虽然...但..."转折稀释成中性。
        if sentiment == "positive" and focus:
            for dim, hint in _POSITIVE_BEHAVIOR_HINT.items():
                if dim in focus:
                    lines.append(f"    【正向必写】{hint}")
                    break
    if note:
        lines.append(f"  · 一句话评语：「{note}」→ 必须用具体行为描述呼应，不要复述原话")
...
def _build_excellent_template_messages(preset, student_name, preferred_name, lesson_info,
    ...
    # 【教师本节课真实表现】前置 —— 与主流程（main）同样的修复：
    # 把 tag/note 提前到 user 消息最前，避免弱模型在 1500 字课件后把它们当"附加信息"漏掉。
    # 同步升级：自动给每个标签打语义标记（↘待改进/↗正向/·中性）+ 回扣落点，
    # —— 见模块级 _teacher_signal_lines / _classify_tag / _infer_focus。
    teacher_signal_block = ""
    if tags or one_sentence:
        _tag_line = ("【教师点选标签 · 该生本节课真实表现定调】" + "、".join(tags)) if tags else ""
        _note_line = f"【教师一句话评语】{one_sentence}" if one_sentence else ""
        _signal_lines = _teacher_signal_lines(tags, one_sentence)
        teacher_signal_block = (
            "【教师本节课真实表现 · 最高优先级 · 必须在课评中明确体现】\n"
            "⚠ 你在本次课评中**必须把下述标签和评语直接落到对该生的评价段里**——\n"
            "· 待改进含义的标签（↘）：必须写出对应的具体不足与具体改进方向，"
            "不得用「可提升」「继续加油」之类空话替代，也不得与标签矛盾写正面套话。\n"
            "· 正向含义的标签（↗）：必须写出该标签对应的具体表现细节，不得只用「很棒」「加油」敷衍；\n"
            "  **严禁使用「虽然...但...」「整体不错但...」「仍需进一步...」「偶尔/有时/稍微」等转折或弱化句式**"
            "来描述 ↗ 正向标签——必须直接、具体地写正向表现，不要稀释。\n"
            "· 中性维度标签（·）：作为背景提示，按本节课实际表现自然带过即可。\n"
            "· 一句话评语：必须用具体行为描述呼应它（如「在花茎和叶脉环节容易走神，导致收尾的涂色步骤比较赶」），不要只复述评语原文。\n"
            "· **回扣落点必须用「行为描述句」承接标签**——对↗正向标签，"
            "把\"注意力集中\"写成\"眼睛紧跟老师示范、起笔不犹豫、能持续 X 分钟不分神\""
            "（示例：把\"注意力集中\"写成\"全程紧盯老师运笔、临帖时眼睛不离开字帖、与老师同步起收，下笔连贯不犹豫\"）。\n"
            "**回扣落点指引**（标签 → 应回扣到哪个行为/维度；教师自定义标签也按同样规则自动分析）：\n"
            + (_signal_lines + "\n" if _signal_lines else "")
            + "**优先级**：下列标签/评语 > 课件里的通用建议 > 默认模板。\n"
            + (_tag_line + "\n" if _tag_line else "")
            + (_note_line + "\n" if _note_line else "")
            + "\n"
        )
...
def build_messages(preset, student_name, preferred_name, lesson_info,
    ...
    # 教师标签语义分析（自动给每个 tag 打语义标记 ↘待改进/↗正向/·中性 + 回扣落点）
    # —— 见模块级 _teacher_signal_lines / _classify_tag / _infer_focus。
    teacher_signals_lines = _teacher_signal_lines(
        quick_tags or [], one_sentence if one_sentence else None
    )
    ...
    # 【教师本节课真实表现】前置到 user 消息最前 —— 弱模型 GLM-4-Flash 在看到
    # 1500+ 字课件和大量输出格式约束后，会把中段的 1 行 tag/note 当作"附加信息"漏掉，
    # 输出变成通用模板套话。提前到 title 之后、课件之前，强制模型先记住"老师认为
    # 该生本节课的真实表现"（tag + note），再读课件时优先找与这些表现相关的细节，
    # 输出时更容易回扣到 tag/note。
    #
    # 关键升级：除了列标签，还自动给**每个标签打语义标记**（↘待改进/↗正向/·中性）
    # + 回扣落点（动手能力差 → 剪刀/胶棒/笔触；注意力不集中 → 易走神环节；…）
    # —— 不再依赖"经验清单"，对任何教师自创标签都生效。
    teacher_signal_block = ""
    if tag_block or note_block:
        _tag_line = tag_block.rstrip("\n") if tag_block else ""
        _note_line = note_block.rstrip("\n") if note_block else ""
        teacher_signal_block = (
            "【教师本节课真实表现 · 最高优先级 · 必须在课评中明确体现】\n"
            "下方是老师对「" + call_name + "」本节课的真实评价（含快捷标签 + 一句话评语）。\n"
            "⚠ 你在本次课评中**必须把下述标签和评语直接落到第 ② 段【课后评价】里**——\n"
            "· 对**待改进含义**的标签（↘）：**必须**在第 ② 段里写出对应的**具体不足**"
            "（如「涂色反复叠涂出了轮廓线」「花茎画得歪扭」「握笔控制不稳，线条粗细不匀」）"
            "与**具体改进方向**（如「下周练习时先定好花的中心再往外画」「可在家用剪刀/胶棒做几步实操」），"
            "不得用「可提升」「继续加油」之类空话替代，也不得与标签矛盾写「线条流畅/掌握扎实」这种正面套话。\n"
            "· 对**正向含义**的标签（↗）：**必须**写出该标签对应的**具体表现细节**"
            "（如「线条收放自然、配色均匀、敢于尝试不同颜色组合」），不得只用「很棒」「加油」敷衍；\n"
            "  **严禁使用「虽然...但...」「整体不错但...」「仍需进一步...」「偶尔/有时/稍微」等转折或弱化句式**"
            "来描述 ↗ 正向标签——必须直接、具体地写正向表现，不要稀释。\n"
            "· 对**中性维度**标签（·）：作为背景提示，按本节课实际表现自然带过即可。\n"
            "· 对**一句话评语**：**必须**用具体行为描述呼应它（如「在花茎和叶脉环节容易走神，导致收尾的涂色步骤比较赶」），不要只复述评语原文。\n"
            "· **回扣落点必须用「行为描述句」承接标签**——对↘待改进标签，把\"动手能力差\"转化为\"在剪刀/胶棒/笔触控制/精细动作等实操环节上的具体表现\""
            "（示例：把\"动手能力差\"写成\"线条容易画歪、剪裁不顺手、涂色不够均匀、笔触控制还需再稳一些\"）；"
            "对↗正向标签，把\"注意力集中\"写成\"眼睛紧跟老师示范、起笔不犹豫、能持续 X 分钟不分神\""
            "（示例：把\"注意力集中\"写成\"全程紧盯老师运笔、临帖时眼睛不离开字帖、与老师同步起收，下笔连贯不犹豫\"）；"
            "不要回避这些细节，也不要复述标签原话里的字面。\n"
            "**回扣落点指引 · 关键词必须字面出现在第②段**（标签 → 应回扣到哪个行为/维度，关键词必须在第②段出现至少 1 次）：\n"
            + teacher_signals_lines + "\n"
            + "**优先级**：上述标签/评语 > 课件里的通用建议 > 默认模板。\n"
            + _tag_line + "\n"
            + _note_line + "\n\n"
        )

    # 教师未点任何快捷标签时，要求模型在课评正文后追加一个亮点标签区块，
    # 由模型自己总结 2-3 个短短语（比渲染时从长文硬抠句子更自然、更准）。
    # 该区块会被后端解析后剥离，不会进入最终正文。
    highlight_req = ""
    if not (quick_tags or []):
        highlight_req = (
            "\n【亮点标签输出 · 仅当上方未提供教师快捷标签时执行】\n"
            "上方没有提供「教师本节课快捷标签」，请你在写完 2 段课评后，"
            "另起两个空行，输出一个亮点标签区块，规则如下：\n"
            "· 以单独一行「【AI亮点标签】」开头（这五个字必须原样出现、一字不差）。\n"
            "· 紧接着在下面另起行，输出 2-3 行，每行一个亮点短语，每个短语 2-8 个汉字，"
            "用具体行为/表现概括本节课该生的正向亮点（示例：「专注投入课堂」「大胆尝试混色」「积极完成练习」），"
            "不要写完整句子、不要带标点符号、不要与上方的课评长句重复。\n"
            "· 这些短语会直接展示给家长作标签，务必简洁、正向、具体。\n"
        )

    user_content = (
        f"本节课标题：{title}\n"
        + teacher_signal_block  # 已含 tag/note 与完整回扣落点指引，下面不再重复
