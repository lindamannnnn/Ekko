...
# 全角 → 半角 标点归一化表：教案/adapter 可能把课题名里的半角括号改写成全角
# （如「一般过去时(规则与不规则)」→「一般过去时（规则与不规则）」），
# 而 KB 文件名是半角括号。不归一化会导致 exact/互含匹配双双失败（KB 未命中）。
...
def _norm(s):
    # 统一去空白 + 转小写 + 全角标点转半角，保证课题名匹配对括号形态不敏感。
    return re.sub(r"\s+", "", (s or "").translate(_FULL2HALF).lower())
