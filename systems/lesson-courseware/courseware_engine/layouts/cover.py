...
DEF = LayoutDef(
    ...
    css=(".ly-cover{position:relative;height:100%;display:flex;flex-direction:column;"
         ...
         ".ly-cover .cover-body{z-index:2;padding:0 calc(8% * var(--pad-scale,1))}"
         ".ly-cover .cover-title{font-size:calc(clamp(30px,6vw,64px) * var(--title-scale,1));font-weight:800;margin:0;letter-spacing:calc(2px + var(--title-ls,0px));text-shadow:0 2px 12px rgba(0,0,0,.35)}"
         ...
         ".ly-cover .cover-kick{position:absolute;top:26px;left:26px;color:#fff;font-size:13px;padding:5px 12px;border-radius:var(--radius,999px);font-weight:700;z-index:3}"
         ".ly-cover .cover-ribbon{position:absolute;top:0;right:0;background:rgba(255,255,255,.16);color:#fff;font-size:13px;padding:8px 22px;border-bottom-left-radius:var(--radius,14px);z-index:3}"
         ".ly-cover .decor{position:absolute;opacity:.9;z-index:1}"
         ".ly-cover .decor-seal{top:24px;right:26px}"
         ".ly-cover .decor-seal span{display:inline-block;width:52px;height:52px;line-height:52px;border:3px solid;border-radius:var(--radius,8px);font-size:26px;font-weight:800}"
